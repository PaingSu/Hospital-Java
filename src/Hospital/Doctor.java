...
public class Doctor extends JPanel {
	...
	private DefaultListModel doctorlist;
	private JList list;
	...
	public Doctor() {
		...
		String sql = "SELECT * FROM `staff` where `stype`='Doctor'";
		try {
			Connection c = con.getConnection();
			Statement st = c.createStatement();
			ResultSet rs = st.executeQuery(sql);
			while (rs.next()) {
				doctorlist.addElement(rs.getString("sname"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

		list = new JList(doctorlist);
		list.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		listpanel.setLayout(new BorderLayout());
		listpanel.add(new JScrollPane(list));
		setBorder(new TitledBorder("Doctors List and Informations"));
		...
		list.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				String sql = "SELECT * FROM `staff` as s,birthday as b WHERE `sname`='"
						+ name + "' and s.sid=b.bid ";
				...
				try {
					...
					while (rs.next()) {
						id = rs.getInt("sid");
						jl_name.setText(" -Dr. " + rs.getString("sname"));
						jl_type.setText(" - " + rs.getString("certificate"));
						jl_bdate.setText(" - " + rs.getInt("day") + " / "
								+ rs.getString("month") + " / "
								+ rs.getInt("year"));
