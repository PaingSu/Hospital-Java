...
public class GeneralStaff extends JPanel {
	 ...
	 * @param args
	 */
	private DefaultListModel generalstafflist;
	private JList list;
	...
	private JButton jbt_remove = new JButton("Remove", new ImageIcon(
			"image/removeadmin.png"));
	private int counter=0;
	private int index=0;
	public GeneralStaff() {
		setLayout(new GridLayout(1, 2));
		JTabbedPane jtp = new JTabbedPane();
		generalstafflist = new DefaultListModel();
		...
		list = new JList(generalstafflist);
	...
	public void createDatabae(){
		String sql = "SELECT * FROM `staff` where `stype`='GeneralStaff'";
		try {
			Connection c = con.getConnection();
			Statement st = c.createStatement();
			ResultSet rs = st.executeQuery(sql);
			while (rs.next()) {
				generalstafflist.addElement(rs.getString("sname"));
				counter++;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void checkingDatabase(){
		...
		try{
			...
			if(index!=counter){
				counter=0;
				index=0;
				generalstafflist.removeAllElements();
				createDatabae();
			}
