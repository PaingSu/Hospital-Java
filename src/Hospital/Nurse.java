...
public class Nurse extends JPanel{
	 ...
	 * @param args
	 */
	private DefaultListModel nurselist;
	private JList list1;
	...
	private JButton jbt_remove=new JButton("Remove",new ImageIcon("image/removenurse.png"));
	...
	public Nurse()
	{
		...
		nurselist = new DefaultListModel();
		...
		list1 = new JList(nurselist);
		list1.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		listpanel.setLayout(new BorderLayout());
		listpanel.add(new JScrollPane(list1));
		setBorder(new TitledBorder("Nurses List and Informations"));
		...
		list1.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				Object name = list1.getSelectedValue();
		...
		jbt_remove.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				Object name1 = list1.getSelectedValue();
				int rindex = list1.getSelectedIndex();
	...
	public void createDatabae(){
		String sql = "SELECT * FROM `staff` where `stype`='Nurse'";
		try {
			Connection c = con.getConnection();
			Statement st = c.createStatement();
			ResultSet rs = st.executeQuery(sql);
			while (rs.next()) {
				nurselist.addElement(rs.getString("sname"));
				counter++;
			}
		} catch (Exception e) {
			e.printStackTrace();
